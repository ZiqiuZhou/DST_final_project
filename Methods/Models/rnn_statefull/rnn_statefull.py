...
class rnn_statefull(object):
    ...
    def createModelName(self, params):
        ...
        str_ = "GPU-" * self.GPU + "RNN-" + self.rnn_cell_type
        for key in keys:
    ...
    def __init__(self, params):
        ...
        self.target = tf.placeholder(self.dtype_tf, shape=[None, None, self.input_dim])
        ...
        self.rnn_output = self.mlp_unfolded_layer("rnn_mlp_output", layer_input, input_size, self.input_dim, 'identity', ln)
    ...
    def getBatch(self, sequence, batch_idx):
        ...
        for predict_on in batch_idx:
            ...
            target = sequence[predict_on:predict_on+self.prediction_length]
    ...
    def train_signal_handler(self, sig, frame):
        ...
        print("Saving model.")
        self.saveModel()
        plotTrainingLosses(self, self.rnn_loss_train_vec, self.rnn_loss_val_vec, self.rnn_min_val_error)
        sys.exit(0)
    ...
    def train(self):
        ...
        try:
            ...
            with open(self.train_data_path, "rb") as file:
                ...
                input_sequence = input_sequence[:self.N_used, :input_dim]
                dt = data["dt"]
                del data

            print("##Using {:}/{:} dimensions and {:}/{:} samples ##".format(input_dim, dim, self.N_used, N_all))
            ...
            if not isWallTimeLimit:
                print("## Training converged: saving model... ##")
                self.saveModel()
                plotTrainingLosses(self, self.rnn_loss_train_vec, self.rnn_loss_val_vec, self.rnn_min_val_error)
    ...
    def trainRound(self, round_num, idx_train_on, idx_val_on, n_train_samples, n_val_samples, train_input_sequence, val_input_sequence):
        ...
        rnn_loss_round_train_vec.append(rnn_loss_train)
        rnn_loss_round_val_vec.append(rnn_loss_val)
        ...
        for epoch in range(self.max_epochs):
            ...
            if epoch > self.overfitting_patience + self.training_min_epochs:
                if all(self.rnn_min_val_error < rnn_loss_round_val_vec[-self.overfitting_patience:]):
    ...
    def predictIndexes(self, input_sequence, ic_indexes, dt, set_name):
        ...
        for ic_num in range(num_test_ICS):
            ...
            if ic_num < 3: plotIterativePrediction(self, set_name, target, prediction, rmse, rmnse, ic_idx, dt, target_augment, prediction_augment, self.n_warmup, hidden_states)

        predictions_all = np.array(predictions_all)
        truths_all = np.array(truths_all)
        hidden_states_all = np.array(hidden_states_all)
        ...
        if self.trackHiddenState: plotAttractor(self, set_name, ic_idx, hidden_states_all)
    ...
    def predictSequence(self, input_sequence):
        ...
        warmup_data_target = np.reshape(input_sequence[1:self.sequence_length + self.n_warmup], (1, -1, self.input_dim))

        target = input_sequence[self.sequence_length+self.n_warmup:]
        ...
        for t in range(self.iterative_prediction_length):
            ...
            input_t = np.reshape(data_output[0,0,:], (1, -1, self.input_dim))
            if self.trackHiddenState: hidden_states.append(last_states)
        ...
        return prediction, target, prediction_augment, target_augment, hidden_states
